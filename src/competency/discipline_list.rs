...
pub struct DisciplineListProps {
    ...
    pub on_rating_changed: Callback<(u32, usize, usize, usize, usize)>, // (new rating, competencyId, areaId)
}
    ...
    disciplines
        ...
        .map(|disc| {
            ...
            let on_disc_rating_changed = {
                Callback::from(move |pair: (u32, usize, usize, usize)| {
            ...
            <>
                <h1>{disc.name}</h1>
                <PathList paths={disc.paths.clone()} on_rating_changed={on_disc_rating_changed.clone()}/>
            </>
            }
        })
        .collect()
