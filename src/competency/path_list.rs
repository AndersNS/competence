...
pub struct PathListProps {
    pub paths: Vec<Path>,
    pub on_rating_changed: Callback<(u32, usize, usize, usize)>, // (new rating, competencyId, areaId)
}
...
pub fn path_list(
    ...
    let on_rating_changed = on_rating_changed.clone();
            let on_path_rating_changed = {
                Callback::from(move |pair: (u32, usize, usize)| {
            ...
            <div class="path">
                ...
                <AreaList sub_areas={path.areas.clone()} on_rating_changed={on_path_rating_changed.clone()}/>
            </div>
            }
        })
        .collect()
