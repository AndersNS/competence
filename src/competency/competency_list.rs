...
pub struct CompetencyListProps {
    ...
    pub on_rating_changed: Callback<(u32, usize)>, // (new rating, competencyId)
}
    ...
    competencies
        ...
        .map(|comp| {
            ...
            let on_comp_rating_changed =
                { Callback::from(move |id: u32| on_rating_changed.emit((id, comp.id))) };
            html! {

                <div class="competency">
                    <h4>{format!("{}", comp.name)}</h4>
                    <rating::Rating selected={3} on_click={on_comp_rating_changed.clone()} />
                </div>
            }
        })
        .collect()
